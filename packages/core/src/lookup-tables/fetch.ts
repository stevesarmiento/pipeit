...
export async function fetchAddressLookupTables(
    ...
    const results = await Promise.all(lookupTableAddresses.map(addr => fetchAddressLookupTable(rpc, addr, commitment)));
...
function parseLookupTableData(buffer: Buffer, tableAddress: Address): LookupTableAccountData {
    ...
    if (typeDiscriminator !== 1) {
        throw new LookupTableInvalidError(tableAddress, `Invalid type discriminator: ${typeDiscriminator}, expected 1`);
    }
