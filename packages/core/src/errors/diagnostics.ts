...
export function diagnoseError(error: unknown): ErrorDiagnosis {
    ...
    if (isSolanaError(error, SOLANA_ERROR__TRANSACTION__SIGNATURES_MISSING)) {
        return {
            ...
            suggestion:
                'Ensure all required signers are provided. Check that the signer matches the fee payer and any other signing accounts.',
            logs,
            originalError: error,
        };
    }
    ...
    if (isSolanaError(error, SOLANA_ERROR__INSTRUCTION_ERROR__CUSTOM)) {
        ...
        return {
            ...
            suggestion:
                'Check the program documentation for error code meanings. Review the instruction parameters and account states.',
    ...
    if (isSolanaError(error, SOLANA_ERROR__INSTRUCTION_ERROR__INSUFFICIENT_FUNDS)) {
        return {
            ...
            suggestion:
                'Check account balances. Ensure the source account has enough funds plus transaction fees (~0.000005 SOL).',
            logs,
            originalError: error,
        };
    }
    ...
    if (isSolanaError(error, SOLANA_ERROR__INSTRUCTION_ERROR__PROGRAM_FAILED_TO_COMPLETE)) {
        return {
            ...
            details:
                'The program encountered an error and could not complete execution. This could be due to compute limits or an internal program error.',
