...
type TransactionRpc = Rpc<
    GetLatestBlockhashApi & GetAccountInfoApi & GetEpochInfoApi & GetSignatureStatusesApi & SendTransactionApi
>;
...
export async function quickTransfer(
    ...
    return builder.setFeePayer(opts.feePayer.address).addInstruction(opts.instruction).execute({ rpcSubscriptions });
}
