...
async function waitForSignatureConfirmation(
    ...
    try {
        ...
        const statusPromise = (async () => {
            const { value: statuses } = await rpc.getSignatureStatuses([signature]).send({ abortSignal: signal });
...
async function waitForBlockHeightExceedence(
    ...
    try {
        ...
        const slotNotifications = await rpcSubscriptions.slotNotifications().subscribe({ abortSignal: signal });
...
export class BlockHeightExceededError extends Error {
    ...
    constructor(currentBlockHeight: bigint, lastValidBlockHeight: bigint) {
        super(`Block height exceeded: current ${currentBlockHeight}, last valid ${lastValidBlockHeight}`);
